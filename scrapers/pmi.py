...
def _get(url: str) -> BeautifulSoup | None:
    for attempt in range(3):
        try:
            ...
            print(f"[PMI] Tentative {attempt+1}/3 {url}: {e}")
    return None
...
def get_pmi(departement_code: str, departement_nom: str,
    ...
    dept_slug   = _dept_slug(departement_code, departement_nom)

    city_urls: list[str] = []

    # 1. Index dept -> villes trouvees dans l'index
    dept_url  = _find_dept_url(departement_code, departement_nom)
    if dept_url:
    ...
    # 2. URLs directes depuis les noms de communes (contourne l'index incomplet)
    if communes:
        for c in communes:
            cp_c  = c.get("cp", "")
            nom_c = c.get("nom", "")
            ...
            if not nom_c:
                continue
            direct = f"https://allopmi.fr/pmi/{dept_slug}/{_city_slug(nom_c)}.html"
            if direct not in city_urls:
                city_urls.append(direct)

    print(f"[PMI] {len(city_urls)} URL(s) de ville a verifier")

    # 3. Pour chaque ville, recuperer les liens des pages detail (dedoublonnes)
    detail_links: list[str] = []
    for city_url in city_urls:
        soup_city = _get(city_url)
        if not soup_city:
            continue
